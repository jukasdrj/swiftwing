...
Source lives under `swiftwing/`, grouped by feature rather than a flat list:

```
swiftwing/
├── App/            # SwiftwingApp, RootView
├── Features/       # Camera, Library, ReviewQueue, Onboarding, Settings
├── Services/       # TalariaService, DataSyncActor, OfflineQueueManager, and the other actors
├── Models/         # SwiftData Book and related types
├── UIComponents/
├── Utilities/
├── OpenAPI/        # Committed talaria-openapi.yaml
├── Fonts/
└── Assets.xcassets/
```
...
- **Progress:** HTTP poll `GET /v3/jobs/scans/{jobId}` then `.../results?format=full`
