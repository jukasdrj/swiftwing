...
- Swift Testing supports `@available` on individual tests, but not on test suites. This app deploys to iOS 27, so do not gate the suite on an older OS. If one test truly needs a newer API later, put `@available` on that test only.
