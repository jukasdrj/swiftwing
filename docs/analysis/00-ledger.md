...
| T3 | open | pass 1, confirmed pass 9 | Slide 2 says photos are never stored. They are. Offline scans live in Documents `/OfflineQueue` until upload. In-flight uploads and the rate-limit queue also write temp JPEGs. Copy fix is separate from this review. |
...
| T9 | open | org review | `Services/` contains six empty directories (`Data`, `Image`, `Network`, `Offline`, `Monitoring`, `Shared`). The service files sit flat beside them. `project.pbxproj` still has a group named Shared. |
| T10 | open | org review | `OpenAPIRuntime` and `OpenAPIURLSession` are linked. No Swift file imports them. The client is handwritten. |
| T11 | open | toolchain | Historical docs still say iOS 26 / Swift 6.2 / iPhone 17 Pro Max: `docs/architecture/DESIGN-DECISION.md`, `docs/superpowers/plans/2026-06-09-readiness.md`, `swiftwing/OpenAPI/README.md`, and the AGENTS.md directory tree (it still describes a flat layout). Live build instructions in `CLAUDE.md`, `AGENTS.md`, `START-HERE.md`, `Makefile`, and `.claude/rules/` now say iOS 27, Swift 6.4, iPhone 18 Pro Max. |
...
| T16 | open | pass 5 | Shelf UI cannot point at a detected book. Lite omits `boundingBox`, and `streamAndProcess` never calls `onSegmented`. Do not switch to `format=full` until one real full payload shows whether boxes are 0–1 or pixels. |
...
| T19 | open | pass 8 | Library "review needed" is `spineConfidence < 0.8`. The review queue keys off enrichment status. The same label is two different sets. |
| T20 | open | pass 9 | Rate-limit retries are in-memory temp JPEGs. Process death or the one-hour temp sweep drops them. Only Documents `/OfflineQueue` survives a relaunch. `StreamManager` releases its slot when the upload returns, before the poll. |
